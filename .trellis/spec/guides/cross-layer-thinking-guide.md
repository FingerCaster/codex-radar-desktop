...
## Stable Identity Versus Activation Identity

An async context can return to the same stable value while still representing
a new activation. Source, account, tenant, profile, workspace, and endpoint
switches all need this distinction.

### Checklist: After Adding A Switchable Async Context

- [ ] Model the schedule as `A1 -> B1 -> A2`; do not stop at `A -> B`.
- [ ] Separate the stable cache key (`A`) from publication authority
      (`{ value: A, generation: 2 }`).
- [ ] Capture the full activation token before starting async work and compare
      the full token before committing visible state, events, errors, or
      notifications.
- [ ] Include activation identity in single-flight/deduplication keys so A2
      cannot join A1.
- [ ] Ensure delayed A1 cleanup can remove only A1 resources, never A2's
      current flight or state.
- [ ] Define explicitly whether an inactive result may update a private cache;
      an old epoch of the reactivated same context must not mutate A2 by
      accident.
- [ ] Make selection persistence transactional: failed persistence must not
      advance the activation token or expose an uncommitted context to pollers.
- [ ] Add delayed success, delayed failure, join isolation, cleanup, and
      first-notification-baseline tests for the A-B-A schedule.

For this project's concrete implementation, see
`backend/radar-data-contract.md` under refresh state and publication.
